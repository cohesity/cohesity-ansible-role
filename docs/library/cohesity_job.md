...
|   | time_zone | String | America/Los_Angeles | Specifies the time zone to use when calculating time for this Protection Job (such as the Job start time). The time must be specified in the **Area/Location** format, such as "America/New_York". List of supported timezones can be found using pytz.all_timezones. To install pytz module visit https://pypi.org/project/pytz/ |
