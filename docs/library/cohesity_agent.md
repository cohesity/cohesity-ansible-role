...
| X | **username** | String | | Username with which Ansible will connect to the Cohesity cluster (username used to login to cluster from UI). Domain-specific credentials can be configured as<br>- Domain@username or Domain/username (will be deprecated in future). Not required if *download_uri* is given.|
