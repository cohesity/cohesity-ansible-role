...
- cohesity_job:
    ...
    exclude_vms: <list of vm's to be excluded from an existing or new VMware protection job>
    include_vms: <list of vm's to be included in an existing or new VMware protection job>
```
...
| X | environment | Choice | -**PhysicalFiles**<br>-Physical<br>-VMware<br>-GenericNas | Specifies the environment type (such as VMware or SQL) of the Protection Source this Job is protecting. For Physical sources this value can be 'PhysicalFiles' or 'Physical'. 'PhysicalFiles' for file based protection jobs and 'Physical' for block based protection jobs |
...
|   | paths | Array | | Specifies a list where each element includes includeFilePath, excludeFilePaths, and skipNestedVolumes options. Should be used only when *environment=PhysicalFiles* |
...
|   | exclude_vms | Array | | List of vm names to be excluded from an existing or new VMware protection job. Can be used only when *state=present* | 
|   | include_vms | Array | | List of vm names to be included in an existing or new VMware protection job. Can be used only when *state=present* |
## Outputs
[top](#cohesity-protection-job)
